...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	...
	cr_assert(fragments == ((double)40/4080), "Fragmentation is incorrect!");
...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	...
	double fragments = sf_fragmentation();
	cr_assert(fragments == ((double)20/4080), "Fragmentation is incorrect!");
...
Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
	...
	cr_assert(util == ((double)40/4096), "Utilization is incorrect");
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
	...
	//checking to see if this free gets coalesced with the wilderness
	assert_free_block_count(0,1);
...
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
	...
	//checking to see if this free gets coalesced with the other free
	assert_free_block_count(0,2);
	assert_free_block_count(3376,1);
